...
def main():
    ...
    for group in (domestic, international, byd_ev):
        for it in group: it.pop("published_ts", None)

    output = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "counts": {"domestic": len(domestic), "international": len(international), "byd_ev": len(byd_ev)},
        "domestic": domestic, "international": international, "byd_ev": byd_ev,
    }

    os.makedirs("data", exist_ok=True)
    with open("data/news.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"Saved {len(domestic)} domestic / {len(international)} international / {len(byd_ev)} byd_ev items")
